...
RUN curl ${ATLAS_REPO} -o apache-atlas-1.0.0-sources.tar.gz \
	&& tar xzf apache-atlas-1.0.0-sources.tar.gz \
	&& cd apache-atlas-sources-1.0.0 \
	&& mvn clean -DskipTests package -Pdist,embedded-hbase-solr -B -Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.trafer.Slf4jMavenTransferListener=warn \
